...
########################################################################
class Fut_Kama_RawSignal_Duo(Signal):
    ...
    #----------------------------------------------------------------------
    def load_param(self):
        filename = get_dss() +  'fut/engine/kama_raw/signal_kama_raw_param.csv'
    ...
    #----------------------------------------------------------------------
    def load_var(self):
        filename = get_dss() +  'fut/engine/kama_raw/signal_kama_raw_'+self.type+'_var_' + pz + '.csv'
        if os.path.exists(filename):
            df = pd.read_csv(filename)
...
########################################################################
class Fut_Kama_RawSignal_Kong(Signal):
    ...
    #----------------------------------------------------------------------
    def load_param(self):
        filename = get_dss() +  'fut/cfg/signal_kama_raw_'+self.type+'_param.csv'
        if os.path.exists(filename):
            df = pd.read_csv(filename)
            df = df[ df.pz == get_contract(self.vtSymbol).pz ]
            if len(df) > 0:
    ...
    def calculateIndicator(self):
        ...
        df = pd.DataFrame(r)
        filename = get_dss() +  'fut/engine/kama_raw/bar_kama_raw_'+self.type+ '_' + self.vtSymbol + '.csv'
        if os.path.exists(filename):
            df.to_csv(filename, index=False, mode='a', header=False)
        else:
            df.to_csv(filename, index=False)
...
########################################################################
class Fut_Kama_RawPortfolio(Portfolio):

    #----------------------------------------------------------------------
    def __init__(self, engine, symbol_list, signal_param={}):
        self.name = 'kama_raw'

        Portfolio.__init__(self, Fut_Kama_RawSignal_Duo, engine, symbol_list, signal_param, Fut_Kama_RawSignal_Kong, signal_param)
        #Portfolio.__init__(self, Fut_Kama_RawSignal_Duo, engine, symbol_list, signal_param, None, None)
        #Portfolio.__init__(self, Fut_Kama_RawSignal_Kong, engine, symbol_list, signal_param, None, None)
