...
class Fut_AtrRsiSignal_Duo(Signal):
    ...
    #----------------------------------------------------------------------
    def load_param(self):
        filename = get_dss() +  'fut/cfg/signal_atrrsi'+self.type+'_param.csv'
    ...
    def generateSignal(self, bar):
        ...
        if self.unit == 0:
            ...
            # 即处于趋势的概率较大，适合CTA开仓
            #if self.atr_short > self.atr_long and self.atr_mid < self.atr_long:
            #if self.atr_short > self.atr_mid and self.atr_long < 30:
            if self.atr_short > self.atr_mid:
                # 使用RSI指标的趋势行情时，会在超买超卖区钝化特征，作为开仓信号
                if self.rsiValue > self.rsiBuy and self.can_buy == True and bar.close > self.bollUp:
    ...
    #----------------------------------------------------------------------
    def load_var(self):
        filename = get_dss() +  'fut/check/signal_atrrsi'+self.type+'_var.csv'
    ...
    def open(self, price, change):
        ...
        df = pd.DataFrame(r, columns=['datetime','direction','offset','volume','price','pnl',  \
                                      'intraTradeHigh','intraTradeLow','stop'])
        filename = get_dss() +  'fut/deal/signal_atrrsi'+self.type+'_' + self.vtSymbol + '.csv'
        if os.path.exists(filename):
            df.to_csv(filename, index=False, mode='a', header=False)
        else:
...
class Fut_AtrRsiSignal_Kong(Signal):
    ...
    def generateSignal(self, bar):
        ...
        if self.unit == 0:
            ...
            if self.atr_short > self.atr_mid:
                if self.rsiValue < self.rsiSell and self.can_sell == True and bar.close < self.bollDown:
                    self.cost = bar.close
                    self.stop = 100E4
