...
########################################################################
class Fut_Cci_RawSignal_Duo(Signal):
    ...
    #----------------------------------------------------------------------
    def load_param(self):
        filename = get_dss() +  'fut/engine/cci_raw/signal_cci_raw_param.csv'
    ...
    def set_param(self, param_dict):
        if 'cciWindow' in param_dict:
            self.cciWindow = param_dict['cciWindow']
            print('成功设置策略参数 self.cciWindow: ',self.cciWindow)
        if 'slMultiplier' in param_dict:
            self.slMultiplier = param_dict['slMultiplier']
            print('成功设置策略参数 self.slMultiplier: ',self.slMultiplier)
        if 'hard_stop_ratio' in param_dict:
            self.hard_stop_ratio = param_dict['hard_stop_ratio']
            print('成功设置策略参数 self.hard_stop_ratio: ',self.hard_stop_ratio)
    ...
    def calculateIndicator(self):
        ...
        cci_pre = self.cciValue
        self.cciValue = self.am.cci(self.cciWindow)
        cci_condition  = True if self.cciValue > 100 and cci_pre <= 100 else False
        ...
        r = [ [self.bar.date,self.bar.time,self.bar.close,self.can_buy,self.can_sell,self.cciValue,self.atrValue,self.intraTradeHigh,self.hard_stop,self.stop] ]
        df = pd.DataFrame(r)
        filename = get_dss() +  'fut/engine/cci_raw/bar_cci_raw_'+self.type+ '_' + self.vtSymbol + '.csv'
        if os.path.exists(filename):
    ...
    def load_var(self):
        ...
        filename = get_dss() +  'fut/engine/cci_raw/signal_cci_raw_'+self.type+'_var_' + pz + '.csv'
        if os.path.exists(filename):
            df = pd.read_csv(filename)
...
########################################################################
class Fut_Cci_RawSignal_Kong(Signal):
    ...
    #----------------------------------------------------------------------
    def load_param(self):
        filename = get_dss() +  'fut/cfg/signal_cci_raw_'+self.type+'_param.csv'
        if os.path.exists(filename):
            df = pd.read_csv(filename)
            df = df[ df.pz == get_contract(self.vtSymbol).pz ]
            if len(df) > 0:
    ...
    def calculateIndicator(self):
        ...
        cci_pre = self.cciValue
        self.cciValue = self.am.cci(self.cciWindow)
        cci_condition  = True if self.cciValue < -90 and cci_pre >= -90 else False
        ...
        r = [ [self.bar.date,self.bar.time,self.bar.close,self.can_short,self.can_cover,self.cciValue,self.atrValue,self.intraTradeLow,self.hard_stop,self.stop] ]
        df = pd.DataFrame(r)
        filename = get_dss() +  'fut/engine/cci_raw/bar_cci_raw_'+self.type+ '_' + self.vtSymbol + '.csv'
        if os.path.exists(filename):
            df.to_csv(filename, index=False, mode='a', header=False)
...
########################################################################
class Fut_Cci_RawPortfolio(Portfolio):

    #----------------------------------------------------------------------
    def __init__(self, engine, symbol_list, signal_param={}):
        self.name = 'cci_raw'

        Portfolio.__init__(self, Fut_Cci_RawSignal_Duo, engine, symbol_list, signal_param, Fut_Cci_RawSignal_Kong, signal_param)
        #Portfolio.__init__(self, Fut_Cci_RawSignal_Duo, engine, symbol_list, signal_param, None, None)
        #Portfolio.__init__(self, Fut_Cci_RawSignal_Kong, engine, symbol_list, signal_param, None, None)
